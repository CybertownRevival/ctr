            ...
            <div class="flex-grow p-1 overflow-y-auto h-full" ref="chatArea">
                <ul>
                    <li v-for="(msg,key) in messages" :key="key">
                        <strong v-if="msg.type && msg.type === 'system'" class="text-white">{{ msg.msg }}</strong>
                        <span v-else>{{msg.userName }}: {{ msg.msg }}</span>
                    </li>
                </ul>

            </div>
            <div class="flex flex-none flex-row space-x-0.5 bg-black" v-if="connected">
                <input type="text" v-model="message" class="flex-grow p-0.5 text-black" @keyup.exact.enter="sendMessage" maxlength="255">
                <button type="button" class="flex-none p-1 bg-gray-300 text-black hover:bg-gray-200 active:bg-gray-400" @click="sendMessage">Send</button>
            </div>
        </div>
        <div class="flex flex-none flex-col w-60 space-0.5 bg-black">
            <div class="flex flex-none p-1 messages-pane text-yellow-200 text-center font-bold">
                <span v-if="activePanel === 'users'" class="flex-grow">({{ this.users.length+1 }}) {{ place.name }}</span>
                <span v-if="activePanel === 'gestures'" class="flex-grow">Body Language</span>
                <span v-if="activePanel === 'sharedObjects'" class="flex-grow">Objects</span>
                <button type="button" class="flex-none p-0.5 bg-gray-300 text-black hover:bg-gray-200 active:bg-gray-400" @click="changeActivePanel">Next</button>
            </div>
            <div class="flex-grow overflow-y-auto p-1 messages-pane">
                <ul v-if="activePanel === 'users'">
                    <li class="text-white"><img src="/assets/img/av_me.gif" class="inline"/> {{ this.$store.data.user.userName }}</li>
                    <li v-for="(user,key) in users" :key="key"><img src="/assets/img/av_invis.gif" class="inline"/>{{user.userName }}</li>
                </ul>
                <ul v-if="activePanel === 'gestures'">
                    <li v-for="(gesture,key) in this.$store.data.user.avatar.gestures" :key='key' @click="sendGesture(key)">{{ gesture }}</li>
                </ul>
                <ul v-if="activePanel === 'sharedObjects'">
                    <li v-for="(object) in sharedObjects" :key='object.id' @click="moveObject(object.id)">{{ object.name }}</li>
                </ul>
            </div>
        </div>
    </div>
    ...
    methods: {
        ...
        sendMessage() {
            ...
            if(this.message !== '' && this.connected) {
                ...
                this.$http.post('/message/place/'+this.place.id,{
                    'body': this.message
                })
                .then(response => {
        ...
        startChat() {
            this.$http.get('/message/place/'+this.place.id, {
                'limit': 10,
                'order': 'id',
                'orderDirection': 'desc'
            })
