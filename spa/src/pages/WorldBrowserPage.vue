		...
		methods: {
			...
			getPlace() {
				...
				return Promise.all([
					...
					this.$http.get(
						"/place/" + this.$route.params.id + "/object_instance"
					),
				]).then((response) => {
			...
			startListeners() {
				...
				X3D.getBrowser().addBrowserCallback({}, (eventType) => {
					...
					if (eventType === X3D.X3DConstants.INITIALIZED_EVENT) {
						this.debugMsg(
							"initalized_event matched! " + this.place.name
						);
						this.debugMsg(this.place);
						let browser = X3D.getBrowser();
						let browserProto = Object.getPrototypeOf(browser);
						let prox =
							browser.currentScene.createNode("ProximitySensor");
			...
			startSharedEvents() {
				...
				this.TYPES = {
					...
					color: {
						...
						fromJSON: (color) =>
							new X3D.SFColor(color.r, color.g, color.b),
					},
					float: {
						toJSON: (e) => e,
						fromJSON: (e) => e,
					...
					vec3f: {
						...
						fromJSON: (vec3f) =>
							new X3D.SFVec2f(vec3f.x, vec3f.y, vec3f.z),
					},
				};
				try {
					sharedZone =
						X3D.getBrowser().currentScene.getNamedNode("SharedZone");
				} catch (e) {
					return;
				}
			...
			handleSharedEvent(e) {
				...
				for (let node of this.eventNodeMap.get(eventObj.name)) {
					node[eventObj.type + "FromServer"] = this.TYPES[
						eventObj.type
					].fromJSON(eventObj.value);
				}
			},
			...
			async addAvatar(e) {
				...
				const loadInlineAsync = (browser, url) => {
					...
					let promise = new Promise((resolve, reject) => {
						loadSensor.addFieldCallback(
							"isLoaded",
							callbackKey,
							(value) => {
								loadSensor.removeFieldCallback(
									"isLoaded",
									callbackKey
								);
								if (value) {
				...
				if (
					...
					loadInlineAsync(browser, avURL).then((avInline) => {
						...
						browser.currentScene.updateImportedNode(
							avInline,
							"Avatar",
							uniqueID
						);
						var avImport =
							browser.currentScene.getImportedNode(uniqueID);
						...
						if (this.users[eventData.id]["inline"]) {
							if (
								...
								this.users[eventData.id]["import"].set_position =
									new X3D.SFVec3f(
										...this.users[eventData.id].transform.pos
									);
							}
							if (
								...
								this.users[eventData.id]["import"].rotation =
									ROTATE180.multiply(
										new X3D.SFRotation(
											...this.users[eventData.id].transform
												.rot
